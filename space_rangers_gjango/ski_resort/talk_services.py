...
def __statistics(game):
    return messages.statistics.format(
        popularity=game.popularity,
        cr=game.cr,
        hotel_places=services.__hotel_places(game),
        restauran_places=services.__restauran_places(game),
        all_lift_places=services.__all_lift_places(game),
        total_people=services.__total_people(game),
        newbie=game.newbie,
        professional=game.professional,
        track_newbie_places=services.__track_newbie_places(game),
        track_professional_places=services.__track_professional_places(game)
    )


def __talk(game, text, values):
    if values == 'Поговорить с бухгалтером':
        return text.format(
            income=(
                services.__profit_newbie(game)
                + services.__profit_professional(game)
            ),
    ...
    if values == 'Поговорить с экономистом':
        return text.format(
            ...
            professional=game.professional,
            hotel=int(
                services.__total_people(game)
                * 100 / services.__hotel_places(game)
            ),
            restauran_places=services.__restauran_places(game),
            all_lift_places=services.__all_lift_places(game),
            track_newbie_places=services.__track_newbie_places(game),
            track_professional_places=(
                services.__track_professional_places(game))
        )
...
def __talk_building(game, index):
