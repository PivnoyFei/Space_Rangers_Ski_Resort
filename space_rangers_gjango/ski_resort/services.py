...
def __track_professional_places(game):
    return (
        game.middle_track * settings.MIDDLE_TRACK[2]
...
def __get_trail_maintenance(game):
    return (
...
def __got_lift_seat(game):
    if __all_lift_places(game) < __total_people(game):
        return True
    return False


def __got_hotel_seat(game):
    if __hotel_places(game) < __total_people(game):
        return True
    return False


def __got_restauran_seat(game):
    if __restauran_places(game) < __total_people(game):
        return True
    return False


def __got_professional_track_seat(game):
    if __track_professional_places(game) < game.professional:
        return True
    return False


def __got_newbie_track_seat(game):
    if __track_newbie_places(game) < game.newbie:
        return True
    return False


def __get_arrived_left_people(game):
...
def __people(game):
    count = 0
    if __got_hotel_seat(game):
        game.hotel_in_demand = True
        count += __total_people(game) - __hotel_places(game)

    if __got_restauran_seat(game):
        game.restauran_in_demand = True
        count += (__total_people(game) - __restauran_places(game))

    if __got_lift_seat(game):
        game.lift_in_demand = True
        count += (__total_people(game) - __all_lift_places(game))

    if __got_newbie_track_seat(game):
        game.low_track_in_demand = True
        count_newbie = (game.newbie - __track_newbie_places(game))
        game.popularity -= count_newbie
        game.newbie -= count_newbie
        game.left_newbie += count_newbie

    if __got_professional_track_seat(game):
        game.high_track_in_demand = True
        count_newbie = (game.professional - __track_professional_places(game))
...
def __new_day(game):
    ...
    game.cr_newbie = __profit_newbie(game)
    game.cr_professional = __profit_professional(game)
    game.cr += game.cr_newbie + game.cr_professional
    __get_arrived_left_people(game)
    ...
    game.cr_track = __get_trail_maintenance(game)
    ...
    if not game.finish and game.cr > 1000000:
        if game.popularity > 60:
            ...
            game.popularity -= 20
        game.finish = True
        services.__user_level(game.user, 100)
        game.save()
        return (
    ...
    if game.cr > 10000000:
        services.__user_level(game.user, 10)
        game.save()
